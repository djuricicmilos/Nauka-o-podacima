...
Iako je Kruskal-Wallis test ukazao na to da postoji statisticki znacajna razlika u raspodeli Premium.Amount varijable u odnosu na nivoe Age.Group varijable, analizom vrednosti efekta, kao i uporedjivanjem medijana i IQR-a, utvrdjujemo da je ova razlika minimalna i prakticno neprimenljiva. Premije osiguranja su uglavnom slične između grupa starosti ≤25, 26–35 i 36–50. Medjutim, grupa 26–35 ima značajno više premije u odnosu na stariju populaciju.
...
Spearmanova korelacija izmedju Premium.Amount i Income.Dependents je statisticki znacajna (ρ = −0.054, p < 0.05), ali izuzetno slaba. S obzirom na veoma veliki uzorak, statisticka znacajnost ne implicira prakticnu vaznost, te se zakljucuje da Income.Dependents nema relevantan uticaj na Premium.Amount.
...
Spearmanova korelacija izmedju Premium.Amount i Income.Credit je statisticki znacajna (ρ = −0.071, p < 0.05), ali vrlo slaba. S obzirom na izuzetno veliki uzorak, statisticka znacajnost ne implicira prakticnu relevantnost, te se zakljucuje da Income.Credit ima ogranicen samostalni uticaj na iznos premije.
...
Iako je Kruskal–Wallis test ukazao na to da postoji statistički značajna razlika u raspodeli varijable Premium.Amount u odnosu na nivoe varijable Income.Group, analizom vrednosti efekta, kao i poređenjem medijana i IQR-a, utvrđujemo da je ova razlika minimalna i praktično zanemarljiva.
...
Iako je Kruskal–Wallis test ukazao na to da postoji statistički značajna razlika u raspodeli varijable Premium.Amount u odnosu na nivoe varijable Health.Group, analizom vrednosti efekta, kao i poređenjem medijana i IQR-a, utvrđujemo da je ova razlika minimalna i praktično zanemarljiva.
...
Spearmanova korelacija izmedju godine ugovaranja polise i iznosa premije je statisticki znacajna (ρ = −0.01, p < 0.05), ali zanemarljive jacine. Rezultat ukazuje da promena godine ugovaranja nema prakticno relevantan uticaj na visinu premije.
...
Spearmanova korelacija izmedju Health.Claims i Premium.Amount je statisticki znacajna (ρ = 0.01517, p < 0.05), ali zanemarljive jacine. Rezultat ukazuje da kombinacija ocene zdravlja i broja prethodnih zahteva nema prakticno relevantan uticaj na visinu premije.
...
Spearmanova korelacija izmedju Health.Age i Premium.Amount je statisticki znacajna (ρ = 0.01038015, p < 0.05), ali zanemarljive jacine. Rezultat ukazuje da kombinacija ocene zdravlja i broja godina nema prakticno relevantan uticaj na visinu premije.
...
corr <- cor(num.data, use = "complete.obs", method = "spearman")
...
```{r}
list.feature.cat.ordinal <- c("Number.of.Dependents", "Education.Level", "Insurance.Duration", "Exercise.Frequency", "Age.Group", "Income.Group", "Health.Group")
...
rec <- recipe(Premium.Amount ~ ., data = data) %>%
  step_ordinalscore(all_of(list.feature.cat.ordinal)) %>%
  step_dummy(all_of(list.feature.cat.onehot))

rec.prep <- prep(rec, training = data)
data.encoded <- bake(rec.prep, new_data = data)
...
preproc <- preProcess(
  data.encoded[, scale.cols],
  method = c("center", "scale")
)

data.scaled <- data.encoded
data.scaled[, scale.cols] <- predict(preproc, data.scaled[, scale.cols])
```

# Podela na train i test skup

```{r}
train.preproc <- data.scaled[!is.na(data.scaled$Premium.Amount), ]

test.preproc <- data.scaled[is.na(data.scaled$Premium.Amount), ]
```

# Cuvanje train i test skupa

```{r}
write.csv(train.preproc, "train.preproc.csv", row.names = FALSE)
write.csv(test.preproc, "test.preproc.csv", row.names = FALSE)
```
