...
## Insurance.Duration prema Policy.Type varijabli

Proveravamo procentualnu zastupljenost po kategorijama
```{r}
tbl <- table(train$Policy.Type, train$Insurance.Duration)
...
Graficka reprezentacija varijabli
```{r}
ggplot(train, aes(x = Policy.Type, fill = Insurance.Duration)) +
  geom_bar(position = "fill") +
  theme_minimal() +
  labs(title = "Procentualna zastupljenost Insurance.Duration po Policy.Type", x = "Policy.Type", fill = "Insurance.Duration",y = "Procenat")
```

Chi_squared test radi statisticke provere znacajnosti izmedju kategorija:
```{r}
p_value <- chisq.test(table(train$Policy.Type, train$Insurance.Duration))$p.value

if (p_value > 0.05) {
  print("Ne postoji statistički značajna zavisnost između Policy.Type i Insurance.Duration.")
} else {
  print("Postoji statistički značajna zavisnost između Policy.Type i Insurance.Duration.")
}
```
